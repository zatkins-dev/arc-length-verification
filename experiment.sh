...
AL_ARGS="-control_type arclength -snes_type newtonal -snes_newtonal_step_size 20"
...
for load in "-5e3" "-12e3"; do
  # echo "SNESNEWTONLS, LU"
  # SUFFIX="newton_lu_${load}"
  # "$EXE" -options_file common.yml -ts_dt 0.05 -ksp_type preonly -pc_type lu -control_type load -ploading $load $MONITOR_OPTIONS_LS > "$(outfile)"
  # postrun

  echo "SNESNEWTONLS"
  SUFFIX="newton_${load}"
  "$EXE" -options_file common.yml -ts_dt 0.05 -control_type load -ploading $load $MONITOR_OPTIONS_LS > "$(outfile)"
  postrun
