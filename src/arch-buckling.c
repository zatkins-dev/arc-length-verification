...
int main(int argc, char **argv) {
  ...
  if (view) {
    ...
    PetscCall(PetscViewerVTKOpen(PETSC_COMM_WORLD, "arch-buckling.vts", FILE_MODE_WRITE, &viewer));
    PetscCall(VecView(C, viewer));
...
void DeformationGradient(Field *ex, PetscInt qi, PetscInt qj, PetscInt qk, PetscScalar *invJ, PetscScalar *F) {
  ...
  F[4] = 1.;
  F[8] = 1.;
  /* form the deformation gradient at this basis function -- loop over element
   * unknowns */
  for (kk = 0; kk < NB; kk++) {
    for (jj = 0; jj < NB; jj++) {
      for (ii = 0; ii < NB; ii++) {
    ...
    if (rank == r) {
      if (j >= ys && j < ys + ym && k >= zs && k < zs + zm) {
        for (i = xs; i < xs + xm; i++) {
          ...
          PetscCall(
              PetscPrintf(PETSC_COMM_SELF, "%" PetscInt_FMT " %d %d: %f %f %f\n", i, 0, 0, (double)coords[0], (double)coords[1], (double)coords[2]));
        }
      }
    }
...
PetscErrorCode SolveArcLengthControl(DM dm, AppCtx *user, Vec *X_solution) {
  ...
  PetscCall(SNESSolve(snes, b, X_initial));
